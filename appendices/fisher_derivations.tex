\section{Derivation of the Phenotypic-Assortment Model}
\label{app:fisher}
...
\label{app:model-components}

For person $i$, define the two column vectors
\[
 X_i=\underbrace{\begin{pmatrix}A_i\\E_i\end{pmatrix}}_{\text{random components}},
 \qquad
 q=\underbrace{\begin{pmatrix}h\\e_P\end{pmatrix}}_{\text{common parameters}},
 \qquad P_i=q^{\mathsf T}X_i=hA_i+e_P E_i.
\]
...
matrix, and
\[
 \operatorname{Var}(P_i)=h^2+e_P^2+2h e_P\operatorname{Cov}(A_i,E_i)=1.
\]
...
The substantive interpretation of the environmental exclusions is in
...
\[
 ...
 =\frac{h\operatorname{Var}(A_i)+e_P\operatorname{Cov}(A_i,E_i)}{1}=h.
\]
...
assumption. From $e_P E_i=P_i-hA_i$ it follows that
\[
 \mathbb E[E_i\mid P_i]
 =\frac{P_i-h\mathbb E[A_i\mid P_i]}{e_P}
 =\frac{1-h^2}{e_P}P_i=e_P P_i,\qquad
...
Because $e_P>0$, replacing each $E_i$ by $P_i=hA_i+e_P E_i$ is an invertible
...
\begin{equation}
 ...
 =\rho qq^{\mathsf T}
 =\rho\begin{pmatrix}h^2&h e_P\\h e_P&e_P^2\end{pmatrix}.
 \label{eq:ge-cross}
\end{equation}
Thus $\operatorname{Cov}(E_M,E_F)=\rho e_P^2$ and
$\operatorname{Cov}(A_M,E_F)=\operatorname{Cov}(E_M,A_F)=\rho h e_P$.
...
\begin{equation}
 A_j=\tfrac12(A_M+A_F)+S_j,\qquad P_j=hA_j+e_P E_j.
...
\begin{align*}
 ...
 &=h\operatorname{Cov}(A_M,A_F)+e_P\operatorname{Cov}(E_M,A_F)\\
 &=h(\rho h^2)+e_P(\rho h e_P)
   =\rho h(h^2+e_P^2)=\rho h.
\end{align*}
...
\begin{equation}
 ...
 =h\operatorname{Cov}(P_M,A_j)+e_P\operatorname{Cov}(P_M,E_j)
...
\[
 ...
 \begin{pmatrix}(1+\rho h^2)/2&0\\\rho h e_P/2&0\end{pmatrix}.
\]
...
\begin{align}
 ...
 &=h^2\operatorname{Cov}(A_1,A_2)
   +h e_P\operatorname{Cov}(A_1,E_2)\notag\\
 &\quad+e_P h\operatorname{Cov}(E_1,A_2)
   +e_P^2\operatorname{Cov}(E_1,E_2)\notag\\
...
Adding sibling environmental covariance $c$ would add $e_P^2c$ if the
...
\begin{align}
 ...
 &=\underbrace{\tfrac12(1+\rho h^2)}_{k}A_i
   +\underbrace{\tfrac12\rho h e_P}_{d}E_i
   =kA_i+dE_i.
...
\begin{align*}
 ...
 &=h\{k\operatorname{Cov}(P_g,A_i)
       ...
       +e_P\operatorname{Cov}(P_g,E_{c(i)})\\
...
\[
 ...
 =h\{k(hk)+d\cdot0\}+e_P\cdot0=\boxed{h^2k^2}.
\]
...
when dominance is omitted. The first three rows require the nuclear-family
assumptions; the extended-relative rows also require the outside-mate
conditions in Section~\ref{sec:model-extended}.
\begin{table}[htbp]
\centering
...
\begin{tabular}{lll}
\hline
Relationship & Correlation & Random mating\\
\hline
Spouses & $\rho$ & $0$\\
Parent--child & $h^2(1+\rho)/2$ & $h^2/2$\\
Full siblings & $h^2k$ & $h^2/2$\\
Grandparent--grandchild & $h^2(1+\rho)k/2$ & $h^2/4$\\
Aunt/uncle--niece/nephew & $h^2k^2$ & $h^2/4$\\
First cousins & $h^2k^3$ & $h^2/8$\\
\hline
\end{tabular}
\end{table}
...
$\boldsymbol\psi=(h^2,\rho)$, not three independent parameters
...
The implications for identification, specification testing, and bias
