...
\item[$\underline x=(x_1,x_2,x_3)'$] an individual's component vector; $\underline a=(1,1,1)'$ so that $y=\underline x'\underline a$. Written out with each entry labeled by what it is:
\begin{equation*}
\underline x \;=\;
\left\{\begin{array}{ll}
x_1 & \text{(additive genotype)} \\
x_2 & \text{(dominance deviation)} \\
x_3 & \text{(environment)}
\end{array}\right\} .
\end{equation*}
\item[$\Sigma=E(\underline x\,\underline x')$] the (diagonal, by A1) variance matrix of an individual's components: $\Sigma=\mathrm{diag}(\sigma_1^2,\sigma_2^2,\sigma_3^2)$. Every off-diagonal entry is zero because A1 says the three components of the \emph{same} person don't covary with each other; only the diagonal (each component's covariance with itself, i.e.\ its variance) survives.
\item[$\underline\pi=E(\underline x\,y)=\Sigma\underline a=(\sigma_1^2,\sigma_2^2,\sigma_3^2)'$] the vector of covariances of an individual's own components with his own phenotype. Since $\Sigma$ is diagonal, this collapses to just the three variances, with the same labeling as $\underline x$ above:
\begin{equation*}
\underline\pi \;=\;
\left\{\begin{array}{ll}
\pi_1 = \sigma_1^2 & \text{(additive genotype's own covariance with own phenotype)} \\
\pi_2 = \sigma_2^2 & \text{(dominance deviation's own covariance with own phenotype)} \\
\pi_3 = \sigma_3^2 & \text{(environment's own covariance with own phenotype)}
\end{array}\right\} .
\end{equation*}
Intuitively, $\pi_i$ answers: ``if I know your phenotype went up by one unit, how much of that do I expect came from component $i$?'' Since phenotype is just the sum of the three components (A1), and they don't covary with each other, the honest answer is ``exactly $\sigma_i^2$ of it, on average'' — no more, no less. This is why $\underline\pi$ recurs everywhere below: it is the fixed ``recipe'' by which any information about someone's total phenotype gets divided back up among his three underlying components.
...
\textbf{Writing this out so it is visible what multiplies what.} Row $i$ of $\Theta$ corresponds to spouse 1's component $i$; column $j$ corresponds to spouse 2's component $j$. Every one of the nine entries is $m$ times (spouse 1's $\pi_i$) times (spouse 2's $\pi_j$) — nothing more exotic than that:
\begin{equation*}
\Theta \;=\;
\left\{\begin{array}{ccc}
\mathrm{Cov}(x_{1,1},x_{1,2}) & \mathrm{Cov}(x_{1,1},x_{2,2}) & \mathrm{Cov}(x_{1,1},x_{3,2}) \\
\mathrm{Cov}(x_{2,1},x_{1,2}) & \mathrm{Cov}(x_{2,1},x_{2,2}) & \mathrm{Cov}(x_{2,1},x_{3,2}) \\
\mathrm{Cov}(x_{3,1},x_{1,2}) & \mathrm{Cov}(x_{3,1},x_{2,2}) & \mathrm{Cov}(x_{3,1},x_{3,2})
\end{array}\right\}
\;=\;
m\left\{\begin{array}{ccc}
\pi_1\pi_1 & \pi_1\pi_2 & \pi_1\pi_3 \\
\pi_2\pi_1 & \pi_2\pi_2 & \pi_2\pi_3 \\
\pi_3\pi_1 & \pi_3\pi_2 & \pi_3\pi_3
\end{array}\right\}
\end{equation*}
with row labels (additive genotype, dominance, environment) reading down spouse 1's side and the same three labels reading across spouse 2's side. The diagonal entries are the ``like-with-like'' covariances (spouse 1's genotype with spouse 2's genotype, etc.); the off-diagonal entries are the ``cross'' covariances (spouse 1's genotype with spouse 2's environment, and so on) that the classical model's critics find most objectionable, precisely because nothing in ordinary experience suggests a bride's genes should predict her groom's upbringing, yet the algebra forces it once you accept that they chose each other by resembling one another overall.
...
Sit with what this second bullet actually says, because it is the single strangest implication of the classical model, and it is worth stating plainly rather than just algebraically. Two people meet, fall for each other on the basis of some overall impression of similarity — similar tastes, similar bearing, similar `type' — and marry. The model says that by doing nothing more than that, they have thereby guaranteed that the wife's \emph{childhood} environment, something that was fixed years before either of them met the other and has nothing causally to do with whom she eventually marries, is now statistically predictable from the husband's. Meanwhile, her own brother's childhood environment — literally the same household she grew up in — carries no such correlation with her husband's, under (A3). A model in which marrying someone links you to their childhood in a way that growing up with them does not is not a natural description of how environments actually get shared; it is an artifact of insisting, by fiat, that assortative mating is the \emph{only} non-genetic channel the model is allowed to use. Every subsequent derivation in this appendix inherits this same peculiarity, because the Lemma just proved is the engine that runs all of them.
...
In plain terms: every generation, Mendelian segregation reshuffles the genetic deck and injects some fresh, unpredictable variation ($\sigma_4^2$) into the additive genotype. Left alone, this would make each generation's genotypic spread wider than the last. What keeps the spread from exploding is that positive assortment ($A>0$) is simultaneously working in the opposite direction, concentrating like with like and thereby manufacturing some of the very variance the population needs in order to stay at a constant $\sigma_1^2$ generation after generation. Equation (4.1) is the exact accounting: the more assortment does this work for you (larger $A$), the less fresh segregation variance is needed to keep the system in balance, and the formula says precisely how much less.
...
It is worth pausing on why the cancellation happens, in words rather than symbols, because it is the single most consequential fact in this whole appendix. Two channels connect a parent to a child: the parent's own half of the genotype (a pure $A$-free transfer, since it is literally the same genetic material passed on), and the \emph{other} parent's half, which only enters because the two parents chose each other, i.e.\ via the spousal Lemma — but the Lemma's correlation is stated in terms of $m$, the raw phenotypic resemblance the couple was actually selected on, not $A$, the narrower genotypic resemblance that resemblance happens to imply. When you add the two channels together, the messy $A$-terms produced along the way happen to cancel exactly, leaving a clean $m$. Nothing here required them to cancel; it is a special feature of this particular relationship (one parent, one step of transmission) and, as Section 8 shows, the cancellation does \emph{not} happen for siblings, which is exactly why parent-offspring and sibling correlations end up depending on different parameters ($m$ versus $A$) even though both are, informally, ``as close as it gets'' in a family tree.
...
\begin{equation*}
\footnotesize
\underline\lambda_{\mathrm{sib}} \;\equiv\; \mathrm{Cov}(\underline x_o,y) \;=\;
\left\{\begin{array}{ll}
d_1 & \text{(sibling's additive genotype, w.\ my whole phenotype)} \\
d_2 & \text{(sibling's dominance, w.\ my whole phenotype)} \\
0 & \text{(sibling's environment, w.\ my whole phenotype --- no channel)}
\end{array}\right\} ,
\end{equation*}
the vector of covariances between one full sibling's three components and the other sibling's \emph{total phenotype} $y$. (Each entry follows immediately from (6.1)/(6.2) since, by A8, $x_{i,o}$ correlates with $y=x_1+x_2+x_3$ only through the matching-index component of $y$.) This vector is the building block for every collateral (non-lineal) relationship computed in the sections that follow — think of it as a portable summary of ``how much of me is visible in my sibling,'' broken down by channel, that can be handed off and reused whenever the pedigree branches sideways instead of straight down.
...
\textbf{Writing this out.} As in Section 3, it helps to see all nine entries at once. Rows index $Z$'s three components ($\lambda_1,\lambda_2,\lambda_3$); columns index the new spouse $V$'s three components ($\pi_1,\pi_2,\pi_3$):
\begin{equation*}
\mathrm{Cov}(\underline x_Z,\underline x_V) \;=\;
m\left\{\begin{array}{ccc}
\lambda_1\pi_1 & \lambda_1\pi_2 & \lambda_1\pi_3 \\
\lambda_2\pi_1 & \lambda_2\pi_2 & \lambda_2\pi_3 \\
\lambda_3\pi_1 & \lambda_3\pi_2 & \lambda_3\pi_3
\end{array}\right\} .
\end{equation*}
Compare this with Section 3's $\Theta=m\,\underline\pi\,\underline\pi'$: the structure is identical (row entry times column entry times $m$), except the row vector is now the more general $\underline\lambda$ (``how $Z$ relates to $W$'s phenotype'') in place of $\underline\pi$ (``how $W$ relates to his own phenotype''). Every derivation from here on needs only \emph{one} column of this matrix at a time — specifically column 1 ($j=1$, the new spouse's additive genotype), since that is the only component of $V$ that ever gets transmitted onward to a child.
...
\begin{equation*}
\footnotesize
\underline\nu \;\equiv\; \mathrm{Cov}(\underline x_G,x_{1,p_1}) \;=\;
\left\{\begin{array}{ll}
d_1 & \text{(grandparent's genotype, w.\ }p_1\text{'s genotype)} \\
\tfrac{m}{2}\sigma_1^2\sigma_2^2 & \text{(grandparent's dominance, w.\ }p_1\text{'s genotype)} \\
\tfrac{m}{2}\sigma_1^2\sigma_3^2 & \text{(grandparent's environment, w.\ }p_1\text{'s genotype)}
\end{array}\right\} .
\end{equation*}
This is simply equations (5.1)--(5.3) relabeled: Section 5 already worked out how each of a parent's three components covaries with the child's additive genotype, for a generic parent-child pair, and $G,p_1$ is just such a pair.
...
\end{equation}
— exactly the Grandparent row of Table 1: the parent-offspring correlation, times one additional factor of $(1+A)/2$ for the extra generation crossed. Notice that this second factor comes in as $A$, not $m$ — the grandparent-to-grandchild link necessarily routes \emph{through} $p_1$'s marriage to $p_2$ (Step 2), and every time the pedigree passes through a marriage rather than a direct birth, it is $A$ (the narrower, genotype-specific echo of assortment) that gets carried forward, not the raw phenotypic $m$ that only shows up when a relationship is a single, unbroken parent-child step (Section 5's special cancellation).
...
Think of it this way: $F$ did not marry a stranger. $F$ married someone (M) who is already partly a genetic and dominance echo of her brother $U$ — that is exactly what it means for $M$ and $U$ to be siblings. Once $F$ resembles $M$ overall (that is what marriage-on-phenotype means), $F$ inherits a faint statistical echo of \emph{everything} $M$ carries, including $M$'s resemblance to $U$. Pretending $F$ is a clean slate with respect to $U$'s family, just because $F$ isn't a blood relative, throws away a real — if secondhand and diluted — connection.
...
\underline\xi \;=\;
\left\{\begin{array}{ll}
d_1 & \text{(uncle's additive genotype, w.\ }M\text{'s additive genotype)} \\
0 & \text{(uncle's dominance, w.\ }M\text{'s additive genotype --- no such channel)} \\
0 & \text{(uncle's environment, w.\ }M\text{'s additive genotype --- no such channel)}
\end{array}\right\} .
\end{equation*}
The difference between $\underline\xi$ and the fuller $\underline\lambda_{\mathrm{sib}}$ is the whole story of this section: $\underline\xi$ is what's relevant when $M$'s additive genotype alone is doing the transmitting (the direct route, exactly like Section 5); $\underline\lambda_{\mathrm{sib}}$, dominance term included, is what's relevant once $M$'s \emph{whole phenotype} becomes the basis on which she gets matched to $F$ (the indirect, marriage-mediated route). The naive shortcut above used only $\underline\xi$ and never noticed the second route existed at all.
